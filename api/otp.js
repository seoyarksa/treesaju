...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'verify') {
      ...
      // 2) profiles — 기존 행만 갱신 (없으면 409), phone은 건드리지 않음
      const nowIso = new Date().toISOString();
      const patch = { phone_verified: true, updated_at: nowIso };

      // 2-1) user_id 기준 update
      ...
      if (upd.error) {
        return json(500, { ok:false, error:'Profile update failed', details: upd.error.message, stage:'update_by_user_id' });
      }
      ...
      // 2-2) (공식 템플릿 대비) id = auth.users.id 기준 update
      upd = await supabase
        .from('profiles')
        .update(patch)
        .eq('id', userId)
      ...
      if (upd.error) {
        return json(500, { ok:false, error:'Profile update failed', details: upd.error.message, stage:'update_by_id' });
      }
      if (Array.isArray(upd.data) && upd.data.length > 0) {
        return json(200, { ok:true, verified:true, via:'update_by_id', profile: upd.data[0] });
      }

      // 2-3) 어떤 키로도 기존 행을 못 찾았으면 생성하지 않고 명확히 안내
      return json(409, {
        ok:false,
