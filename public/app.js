...
const USER_ID_COLUMN = "user_id"; // 사용자 식별 컬럼 (없으면 'id' 사용)
...
// 로컬(0으로 시작) 숫자형
const toLocal010 = (raw) => {
  let d = digitsOnly(raw);
  if (!d) return "";
  if (d.startsWith("82")) d = d.slice(2);
  if (!d.startsWith("0")) d = "0" + d;
  return d; // 예: 01012345678
};

// 다양한 저장 가능성을 커버하는 후보군 생성
function buildPhoneCandidates(raw) {
  const local = toLocal010(raw);                // 01012345678
  const e164  = toE164Kr(raw);                  // +821012345678
  const hyp   = toHyphen(local);                // 010-1234-5678
  const plusHyp = toHyphen(e164.replace(/^\+82/, "0")); // (희귀) +82 기반 하이픈 대응
  const plain = digitsOnly(raw);                // 01012345678 / 821012...

  // 중복 제거
  const set = new Set([local, e164, hyp, plusHyp, plain].filter(Boolean));
  return Array.from(set);
}

// 중복 검사: 여러 형식을 .in()으로 한 번에
async function isPhoneDuplicatedMulti(rawPhone) {
  ...
  const { count, error } = await window.supabaseClient
    ...
    .in(PHONE_COLUMN, candidates);

  if (error) {
    console.error("중복 확인 오류:", error, { candidates });
    throw error;
  }
  return (count || 0) > 0;
}
...
document.getElementById("otp-send")?.addEventListener("click", async (e) => {
  ...
  const local = toLocal010(raw);
  if (!local || digitsOnly(local).length < 10) {
  ...
  try {
    const duplicated = await isPhoneDuplicatedMulti(raw);
    if (duplicated) {
    ...
    // ⬇️ 여기에서 기존 OTP 전송 로직 실행
    // await sendOtp(toE164Kr(raw));
    console.log("중복 아님 → OTP 전송 진행", { e164: toE164Kr(raw) });
    btn.textContent = "전송됨";
...
// E.164(+821012345678) → "010-1234-5678" 포맷 자동채움
function formatKrLocal(phone) {
  if (!phone) return "";
  ...
  // 010-XXXX-XXXX / 011 등도 대략 보정
  if (d.length === 11) return d.replace(/(\d{3})(\d{4})(\d{4})/, "$1-$2-$3");
  if (d.length === 10) return d.replace(/(\d{3})(\d{3,4})(\d{4})/, "$1-$2-$3");
  return d;
}

// DB에서 현재 로그인한 사용자의 전화번호 가져오기
async function fetchMyPhoneFromDB() {
  // 1) 인증 사용자 ID 얻기
  const { data: userData, error: authErr } = await window.supabaseClient.auth.getUser();
  ...
  let q = window.supabaseClient
    .from(PHONE_TABLE)
    .select(`${PHONE_COLUMN}`, { count: "none" })
    .eq(USER_ID_COLUMN, uid)
    .limit(1);

  let { data, error } = await q;
  if (error) {
    console.warn("user_id로 조회 실패, id로 재시도:", error);
  }

  // 3) 없거나 실패하면 id 컬럼으로 재시도
  if (!data || !data.length) {
    const q2 = window.supabaseClient
      .from(PHONE_TABLE)
      .select(`${PHONE_COLUMN}`, { count: "none" })
      .eq("id", uid)
      .limit(1);
    const r2 = await q2;
    if (r2.error) {
      console.error("전화번호 조회 오류:", r2.error);
      return null;
    }
    data = r2.data;
  }
  ...
  return data[0]?.[PHONE_COLUMN] || null;
}

// 입력창에 값/placeholder 채우기
async function prefillOtpPhone() {
  const input = document.getElementById("otp-phone");
  if (!input) return;

  try {
    const saved = await fetchMyPhoneFromDB(); // e.g. "+821012345678" 또는 "01012345678"
    if (!saved) return;

    const pretty = formatKrLocal(saved);

    // 값이 비어있다면 value로 채움 (사용자가 바로 수정 가능)
    if (!input.value) input.value = pretty;

    // 그래도 비어있다면 최소 placeholder에 표시
    if (!input.value && !input.placeholder) {
      input.placeholder = pretty;
    }
  } catch (e) {
    console.error("전화번호 자동 채우기 실패:", e);
  }
}

// Supabase가 준비된 뒤 한 번 실행 (페이지 로드 시점 등에서 호출)
prefillOtpPhone();
...
document.getElementById("gwBtnInicis").addEventListener("click", () => {
  ...
  function renderPurchaseChoices() {
    ...
    document.getElementById("btn3m")?.addEventListener("click", () => openGatewayChooser("3m"));
    document.getElementById("btn6m")?.addEventListener("click", () => openGatewayChooser("6m"));
    document.getElementById("btnRecurringBasic")?.addEventListener("click", () => openGatewayChooser("rb"));
    document.getElementById("btnRecurringPlus")?.addEventListener("click", () => openGatewayChooser("rp"));
