...
// 결제 버튼 클릭 → (전화인증 보장) → 아임포트 결제 → 서버 API 호출 → UI 갱신
(function attachSubscribeHandler() {
  const btn = document.getElementById('subscribeBtn');
  if (!btn || btn.dataset.bound) return;     // ✅ 중복 바인딩 방지
  btn.dataset.bound = '1';
  ...
  function ensureIMP() {
    ...
    if (!IMP) return null;
    if (!IMP_READY) {
      IMP.init('imp_여기에_본인_가맹점코드');  // ✅ 실제 imp_코드로 교체
      IMP_READY = true;
    }
    return IMP;
  }

  // 전화(본인)인증 보장: 이미 인증이면 true, 아니면 인증 창을 띄워 통과 후 true
  async function ensurePhoneVerified(sb, userId) {
    const { data: prof, error } = await sb
      .from('profiles').select('phone_verified').eq('user_id', userId).single();
    if (error) { alert('프로필 조회 오류. 잠시 후 다시 시도해주세요.'); return false; }
    if (prof?.phone_verified) return true;

    const IMP = ensureIMP();
    if (!IMP) { alert('본인인증 모듈 로드 실패'); return false; }

    return new Promise((resolve) => {
      IMP.certification({ merchant_uid: 'cert_' + Date.now() }, async (rsp) => {
        if (!rsp.success) {
          alert('전화(본인) 인증이 취소/실패되었습니다.');
          return resolve(false);
        }
        // (권장: 서버 검증 후 업데이트) — 우선 클라이언트에서 표기 반영
  ...
  btn.addEventListener('click', async (e) => {
    e.preventDefault();
    e.stopImmediatePropagation();           // ✅ 혹시 남아있던 옛 리스너 차단
    ...
    // 1) 전화인증 먼저 보장 (여기서 통과 못하면 결제창을 절대 호출하지 않음)
    const verified = await ensurePhoneVerified(sb, user.id);
    if (!verified) return;
    ...
    btn.disabled = true;
    btn.textContent = '결제 진행중…';

    try {
      const merchantUid = `sub_${user.id}_${Date.now()}`;
      IMP.request_pay({
        // pg: 'html5_inicis',  // ❌ 일단 제거하여 대시보드 기본 PG 사용 (불일치 오류 회피)
        pay_method: 'card',
        ...
        if (!rsp.success) {
          ...
          btn.disabled = false;
          btn.textContent = '정기구독 및 결제';
          return;
        }

        // 3) 서버에 등록 (memberships + profiles.grade 갱신)
